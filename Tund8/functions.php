<?php
	...
	function listAllIdeas() {
		...
		$stmt = $mysqli -> prepare ("SELECT id, idea, color FROM vpusers_ideas WHERE userid = ? AND deleted IS NULL ORDER BY id DESC");
	...
	function latestIdea() {
		...
		$stmt = $mysqli -> prepare("SELECT idea FROM vpusers_ideas WHERE id = (SELECT MAX(id) FROM vpusers_ideas WHERE deleted IS NOT NULL)");
